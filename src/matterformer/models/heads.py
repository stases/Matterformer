...
class CrystalHeads(nn.Module):
    ...
    def _coord_from_relative(
        ...
        if frac_coords.shape[-1] != 3:
            raise RuntimeError(f"frac_coords last dim must be 3, got {frac_coords.shape[-1]}")

        q = self.coord_query(h_atoms)
        k = self.coord_key(h_atoms)
        scores = torch.einsum("bid,bjd->bij", q, k).float() * self.coord_scale

        key_pad_mask = None
        if pad_mask is not None:
            if pad_mask.shape != h_atoms.shape[:2]:
                raise RuntimeError(
                    f"pad_mask shape {tuple(pad_mask.shape)} does not match atoms {tuple(h_atoms.shape[:2])}"
                )
        ...
        coord_vel = torch.einsum("bij,bijc->bic", attn, delta.to(dtype=attn.dtype))
        coord_vel = coord_vel.to(dtype=h_atoms.dtype)
        if pad_mask is not None:
