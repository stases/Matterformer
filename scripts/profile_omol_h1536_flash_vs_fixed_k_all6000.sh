#!/bin/bash
# Profile h1536 OMol all-global FlashAttention vs alternating fixed-K eSEN-local/global architecture.

#SBATCH --partition=all6000
#SBATCH --account=all6000users
#SBATCH --gres=gpu:1
#SBATCH --job-name=mf_h1536_fkprof
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --time=02:00:00
#SBATCH --mem=96G
#SBATCH --output=/home/thadziv/matterformer_jobs/job_outputs/slurm_output_%A_mf_h1536_fkprof.out
...
OUTPUT_DIR="${OUTPUT_DIR:-/home/thadziv/matterformer_runs/benchmarks/omol_h1536_flash_vs_fixedk_${SLURM_RUN_ID}}"
...
export TORCHINDUCTOR_CACHE_DIR="${TORCHINDUCTOR_CACHE_DIR:-/tmp/$USER/torchinductor_matterformer_fixedk_profile}"
...
echo "============================================================"
echo "Matterformer h1536 flash vs fixed-K eSEN OMol forward profile"
...
"$PYTHON_BIN" scripts/profile_omol_architectures.py \
  ...
  --alt-config "$REPO_ROOT/configs/omol/tetra_t_only_h1536_l16_pt2_goodrun_qkn_uk_csfilm_fixed_k_esen_r4_k48_every2.json" \
  --alt-name fixed_k_esen_r4_k48_every2 \
  ...
  --full-forward-warmup "${FULL_FORWARD_WARMUP:-2}" \
  --full-forward-repeats "${FULL_FORWARD_REPEATS:-5}" \
  --layer-warmup "${LAYER_WARMUP:-1}" \
  --layer-repeats "${LAYER_REPEATS:-2}" \
  --component-warmup "${COMPONENT_WARMUP:-1}" \
  --component-repeats "${COMPONENT_REPEATS:-2}" \
  --skip-fwd-bwd \
  --no-skip-component \
...
echo "component_forward_csv:$OUTPUT_DIR/full_model/component_forward.csv"
