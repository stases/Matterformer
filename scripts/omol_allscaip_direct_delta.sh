#!/bin/bash
# OMol 4M FairChem AllScAIP direct-force backend run.

#SBATCH --partition=delta
#SBATCH --account=deltausers
#SBATCH --gres=gpu:1
#SBATCH --job-name=omol_allscaip
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --time=5-00:00:00
#SBATCH --mem=64G
#SBATCH --output=/home/thadziv/matterformer_jobs/job_outputs/slurm_output_%A_omol_allscaip.out
...
ALLSCAIP_OFFICIAL_CONFIG="${ALLSCAIP_OFFICIAL_CONFIG:-$REPO_ROOT/configs/omol/allscaip_omol4m_md_neno_direct.json}"

export RUN_SLUG="${RUN_SLUG:-matterformer_omol4m_allscaip_md_neno_direct_h640_l10_b96_a400_lr8e4_official}"
export WANDB_PROJECT="${WANDB_PROJECT:-matterformer_omol_4m}"
export WANDB_GROUP="${WANDB_GROUP:-matterformer_omol4m_allscaip_md_neno_direct_official}"
...
export ALLSCAIP_CONFIG_JSON="${ALLSCAIP_CONFIG_JSON:-$ALLSCAIP_OFFICIAL_CONFIG}"
export ALLSCAIP_STRICT_CONFIG_JSON="${ALLSCAIP_STRICT_CONFIG_JSON:-$ALLSCAIP_OFFICIAL_CONFIG}"
export ALLSCAIP_HIDDEN_SIZE="${ALLSCAIP_HIDDEN_SIZE:-640}"
export ALLSCAIP_NUM_LAYERS="${ALLSCAIP_NUM_LAYERS:-10}"
export ALLSCAIP_ATTEN_NUM_HEADS="${ALLSCAIP_ATTEN_NUM_HEADS:-10}"
