#!/bin/bash
# Matterformer PT2-style OMol run with a 22-layer 1280-wide trivial-group Platonic trunk and Muon.

#SBATCH --partition=delta
#SBATCH --account=deltausers
#SBATCH --gres=gpu:1
#SBATCH --job-name=mf_pt2_triv1280
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --time=5-00:00:00
#SBATCH --mem=64G
#SBATCH --output=/home/thadziv/matterformer_jobs/job_outputs/slurm_output_%A_mf_pt2_triv1280.out

set -euo pipefail

REPO_ROOT="${REPO_ROOT:-/home/thadziv/GitHub/Matterformer}"

export HYBRID_CONFIG_JSON="${HYBRID_CONFIG_JSON:-$REPO_ROOT/configs/omol/trivial_t_only_h1280_l22_pt2_exact_sin_layerscale.json}"
export MODEL_BACKEND="${MODEL_BACKEND:-matterformer}"
export OMOL_RUNTIME_MODE="${OMOL_RUNTIME_MODE:-internal_flat_tetra}"

export D_MODEL="${D_MODEL:-1280}"
export N_HEADS="${N_HEADS:-32}"
export N_LAYERS="${N_LAYERS:-22}"
...
export RUN_SLUG="${RUN_SLUG:-matterformer-pt2-trivial-h1280-l22-ls1e-4-ffn2-actsin-ractsin-rs2.0-muon-fw100-60ep-n12000-flat-maxg999999-platonichead}"
export WANDB_PROJECT="${WANDB_PROJECT:-matterformer_omol_4m}"
export WANDB_GROUP="${WANDB_GROUP:-matterformer_pt2_trivial_h1280_l22_muon_fw100_platonichead}"
