#!/bin/bash
# Matterformer PT2 OMol run: H2304 L16 + RBF/type local attention bias + irrep invariant readout + pair-force residual.

#SBATCH --partition=delta
#SBATCH --account=deltausers
#SBATCH --gres=gpu:1
#SBATCH --job-name=mf_h2304_irpair_rbf
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --time=5-00:00:00
#SBATCH --mem=96G
#SBATCH --output=/home/thadziv/matterformer_jobs/job_outputs/slurm_output_%A_mf_h2304_irpair_rbf.out

set -euo pipefail

REPO_ROOT="${REPO_ROOT:-/home/thadziv/GitHub/Matterformer}"

export HYBRID_CONFIG_JSON="${HYBRID_CONFIG_JSON:-$REPO_ROOT/configs/omol/tetra_t_only_h2304_l16_pt2_exact_sin_layerscale_rbf_type_every2.json}"
export MODEL_BACKEND="${MODEL_BACKEND:-matterformer}"
export OMOL_RUNTIME_MODE="${OMOL_RUNTIME_MODE:-internal_flat_tetra}"

export D_MODEL="${D_MODEL:-2304}"
...
export CHGSPIN_EMB_DIM="${CHGSPIN_EMB_DIM:-192}"
...
export FLOPS_COEF="${FLOPS_COEF:-103.68}"
...
export RUN_SLUG="${RUN_SLUG:-matterformer-pt2-h2304-l16-ls1e-4-ffn2-actsin-ractsin-rs2.0-ema0.99-wd1e-5-muonffnconv0.01-mwd0-warm5k-auxadamw-40ep-n12000-flat-maxg999999-irrepinv-pairforce30-rbftype-every2-skip1000}"
export WANDB_PROJECT="${WANDB_PROJECT:-matterformer_omol_4m}"
export WANDB_GROUP="${WANDB_GROUP:-matterformer_pt2_h2304_l16_irrepinv_pairforce30_rbftype_every2_muonffnconv0p01_mwd0_warm5k_auxadamw_skip1000}"
