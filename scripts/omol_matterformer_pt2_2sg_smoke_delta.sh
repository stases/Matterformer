#!/bin/bash
# PT2-parity Matterformer OMol smoke run with two Triton grouped S_g insertions.

#SBATCH --partition=delta
#SBATCH --account=deltausers
#SBATCH --gres=gpu:1
#SBATCH --job-name=mf_pt2_2sg
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --time=1-00:00:00
#SBATCH --mem=64G
#SBATCH --output=/home/thadziv/matterformer_jobs/job_outputs/slurm_output_%A_mf_pt2_2sg.out

set -euo pipefail

REPO_ROOT="${REPO_ROOT:-/home/thadziv/GitHub/Matterformer}"

export HYBRID_CONFIG_JSON="${HYBRID_CONFIG_JSON:-$REPO_ROOT/configs/omol/tetra_t_plus_2sg_h1920_l8_pt2_exact_sin_layerscale.json}"
...
export BATCHING_MODE="${BATCHING_MODE:-bucketed}"
...
export VAL_ESTIMATE_EVERY_STEPS="${VAL_ESTIMATE_EVERY_STEPS:-50}"
export VAL_ESTIMATE_BATCHES="${VAL_ESTIMATE_BATCHES:-8}"
export FULL_VAL_EVERY_STEPS="${FULL_VAL_EVERY_STEPS:-200}"
export LIMIT_VAL_BATCHES="${LIMIT_VAL_BATCHES:-100}"
...
export RUN_SLUG="${RUN_SLUG:-matterformer-pt2-h1920-l8-plus2sg-k16-ls1e-4-ffn2-actsin-ractsin-ema0.99-wd1e-5-smoke-n20000-platonichead}"
export WANDB_PROJECT="${WANDB_PROJECT:-matterformer_omol_4m}"
export WANDB_GROUP="${WANDB_GROUP:-matterformer_pt2_plus2sg_k16_smoke_platonichead}"
