#!/bin/bash
# Matterformer h1920/dpf160/l24 with alternating global FlashAttention and
# fixed-K eSEN local attention, launched as 4-process DDP on one Delta node.

#SBATCH --partition=delta
#SBATCH --account=deltausers
#SBATCH --gres=gpu:4
#SBATCH --job-name=mf_l24_fk32_muon
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=64
#SBATCH --time=7-00:00:00
#SBATCH --mem=256G
#SBATCH --output=/home/thadziv/matterformer_jobs/job_outputs/slurm_output_%A_mf_l24_fk32_muon.out
...
export HYBRID_CONFIG_JSON="${HYBRID_CONFIG_JSON:-$REPO_ROOT/configs/omol/tetra_t_only_h1920_l24_pt2_goodrun_qkn_uk_csfilm_fixed_k_esen_r4_k32_every2_platonic_input.json}"
...
export RUN_SLUG="${RUN_SLUG:-mf_pt2_h1920_l24_fkesenR4K32_alt_ddp4_muon0p01_adam3e-4_warm6k_ropefp64}"
export WANDB_PROJECT="${WANDB_PROJECT:-matterformer_omol_4m}"
export WANDB_GROUP="${WANDB_GROUP:-matterformer_pt2_h1920_l24_fixedkR4K32_ddp4_muon0p01_adam3e-4_warm6k_fw20}"
export WANDB_RUN_NAME="${WANDB_RUN_NAME:-matterformer-pt2-h1920-l24-fixedkR4K32-alt-ddp4-muon0p01-adam3e-4-warm6k}"
