#!/bin/bash
# BWDB stage-1 CSP run on delta using MHA attention with geometry bias disabled.
# Mirrors the current 65M-stage y1 launcher but swaps simplicial attention for MHA.

#SBATCH --partition=delta
#SBATCH --account=deltausers
#SBATCH --gres=gpu:1
#SBATCH --job-name=bwdb_s1_65m_mha
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=8
#SBATCH --time=7-00:00:00
#SBATCH --mem=64000M
#SBATCH --output=/home/thadziv/matterformer_jobs/job_outputs/slurm_output_%A_bwdb_s1_65m_mha.out
...
export ATTN_TYPE="${ATTN_TYPE:-mha}"
...
export DISABLE_CUDNN_SDPA="${DISABLE_CUDNN_SDPA:-1}"
...
export RUN_SLUG="${RUN_SLUG:-bwdb_stage1_csp_65m_mha_nogb_y1}"
export WANDB_PROJECT="${WANDB_PROJECT:-Matterformer_BWDB_stage1}"
export WANDB_GROUP="${WANDB_GROUP:-bwdb_stage1_csp_65m_mha_nogb_y1}"
