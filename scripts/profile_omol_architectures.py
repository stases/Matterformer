...
def _layer_metas(model: MatterformerOMolForceField) -> list[LayerMeta]:
    ...
    for name, layer in _iter_tetra_layers(model):
        ...
        radius_cutoff = None
        if bias_kind == "radius_rbf_type_enveloped":
...
def _compile_model(model: MatterformerOMolForceField, *, strategy: str, mode: str) -> dict[str, Any]:
    ...
    if strategy == "run_matched":
        if model.trunk._radius_sparse_layout_config() is not None:
            stats = model.trunk.compile_flat_tetra_layer_forwards(mode=mode)
            return {"strategy": "radius_sparse_layer_selective", **stats}
            ...
            metrics.append(
                {
                    ...
                    "layout_build_ms": sum(float(row["layout_build_ms"]) for row in layout_recorder.records),
                    "layout_calls": float(len(layout_recorder.records)),
                }
            )
    return {
        "name": name,
        ...
        "layout_build_ms": _summary([row["layout_build_ms"] for row in metrics]),
        "layout_calls": _summary([row["layout_calls"] for row in metrics]),
        "raw": metrics,
    }
    ...
    *,
    ...
    try:
        for _ in range(args.layer_warmup):
            ...
            layout_recorder.clear()
            with torch.no_grad():
                _forward_loss(model, criterion, batch, args)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Profile h1536 OMol flash baseline vs radius-sparse alternating architecture.")
    ...
    try:
        for arch_name, config_path in [("flash_baseline", args.baseline_config), ("radius_sparse_alt", args.sparse_config)]:
            print(f"=== {arch_name}: full fwd+bwd run-matched compile ===")
            ...
            print("full " + json.dumps({arch_name: full}, sort_keys=True))
            print("layout " + json.dumps({arch_name: layer["layout"]}, sort_keys=True))
    finally:
        layout_recorder.restore()

    baseline_ms = results["architectures"]["flash_baseline"]["full_fwd_bwd"]["wall_ms"]["mean"]
    sparse_ms = results["architectures"]["radius_sparse_alt"]["full_fwd_bwd"]["wall_ms"]["mean"]
    results["comparison"] = {
        "sparse_vs_flash_fwd_bwd_time_ratio": float(sparse_ms / baseline_ms) if baseline_ms > 0 else None,
        "sparse_extra_fwd_bwd_ms": float(sparse_ms - baseline_ms),
    }
