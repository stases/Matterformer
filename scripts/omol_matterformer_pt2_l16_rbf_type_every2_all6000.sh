#!/bin/bash
# Matterformer PT2 exact-style OMol run with scheduled smooth local RBF/type attention bias.

#SBATCH --partition=all6000
#SBATCH --account=all6000users
#SBATCH --gres=gpu:1
#SBATCH --job-name=mf_l16_rbf_type_e2
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --time=5-00:00:00
#SBATCH --mem=64G
#SBATCH --output=/home/thadziv/matterformer_jobs/job_outputs/slurm_output_%A_mf_l16_rbf_type_e2.out

set -euo pipefail

REPO_ROOT="${REPO_ROOT:-/home/thadziv/GitHub/Matterformer}"

export HYBRID_CONFIG_JSON="${HYBRID_CONFIG_JSON:-$REPO_ROOT/configs/omol/tetra_t_only_h1920_l16_pt2_exact_sin_layerscale_rbf_type_every2.json}"
...
export RUN_SLUG="${RUN_SLUG:-matterformer-pt2-h1920-l16-ls1e-4-ffn2-actsin-ractsin-rs2.0-ema0.99-wd1e-5-muonffnconv0.01-mwd0-warm5k-auxadamw-40ep-n12000-flat-maxg999999-platonichead-rbftype-every2-skip1000}"
export WANDB_PROJECT="${WANDB_PROJECT:-matterformer_omol_4m}"
export WANDB_GROUP="${WANDB_GROUP:-matterformer_pt2_l16_platonichead_rbftype_every2_muonffnconv0p01_mwd0_warm5k_auxadamw_skip1000}"
